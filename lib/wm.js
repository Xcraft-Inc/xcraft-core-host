...
class Window {
  constructor(wm, options) {
    const {BrowserWindow} = require('xcraft-core-host/lib/neutron.js');
    //copy config
    this.wm = wm;
    ...
    const {partition, ...otherWebPreferencies} =
      this.wm?.config?.windowOptions?.webPreferencies || {};
    this.winOptions.webPreferences = {
      nodeIntegration: true,
      contextIsolation: false,
      ...(this.wm?.config?.windowOptions?.webPreferencies
        ? {
            partition: partition?.replaceAll('$PROCESS_PID', process.pid),
            ...otherWebPreferencies,
          }
        : {}),
      // partition: `persist:${process.pid}`, // ensure that a second instance does not start slow. See https://github.com/electron/electron/issues/22438
    };
...
class WindowManager {
  constructor() {
    ...
    const {Menu, app, shell} = require('xcraft-core-host/lib/neutron.js');
    if (!app) {
  ...
  async prompt({values}, listener) {
    const getResult = new Promise((resolve, reject) => {
      this.displaySplash(() => {
        ...
        const {ipcMain} = require('xcraft-core-host/lib/neutron.js');
  ...
  create(windowId, options) {
    ...
    if (this.config?.windowOptions?.contextMenu === true) {
      const {Menu, MenuItem} = require('xcraft-core-host/lib/neutron.js');
  ...
  init(xHost) {
    const {session} = require('xcraft-core-host/lib/neutron.js');
