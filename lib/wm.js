...
class WindowManager {
  ...
  create(windowId, desktopId, feeds, options) {
    ...
    //If we are a window created during splash lifetime
    if (windowId !== 'splash' && this._services['splash'] !== undefined) {
      options.show = false; //will be show manually when splash dispose
    }
    ...
    if (windowId !== 'splash' && this._services.splash) {
      window.win.webContents.once('ready-to-show', () => {
        setTimeout(() => {
          this.dispose('splash');
          window.win.show();
        }, this.config.splashDelay);
      });
    }
    return window;
  }
