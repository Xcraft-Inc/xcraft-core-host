...
class Host extends EventEmitter {
  ...
  *_init(xBus, next) {
    ...
    this.#unsubHostAlert = this._busClient.events.subscribe(
      '*::host-alert-dialog',
      (msg) => {
        const {cmd, alert} = msg.data;
        switch (cmd) {
          case 'close': {
            if (this.#hostAlertDialog) {
              this.#hostAlertDialog.close();
              this.#unsubHostAlert();
            }
            this.#hostAlertDialog = null;
            this.#unsubHostAlert = null;
            break;
          }
          ...
          case 'open': {
            ...
            if (this.#hostAlertDialog) {
              this.#hostAlertDialog.emit(alert);
              this.#hostAlertDialog.moveToFront();
              return;
            }

            const MsgBox = require('./msgbox.js');
            this.#hostAlertDialog = new MsgBox();
            this.#hostAlertDialog.open();
            this.#hostAlertDialog.emit(alert);
            this.#hostAlertDialog.moveToFront();
            break;
          }
