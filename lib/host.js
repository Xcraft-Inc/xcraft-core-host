...
class Host extends EventEmitter {
  ...
  async tryToImportKeys(server) {
    ...
    const result = await neutron.dialogShowMessageBoxSync({
    ...
    const filePaths = await neutron.dialogShowOpenDialog({
    ...
    if (!imported) {
      await neutron.dialogShowErrorBox(
  ...
  async load(initialConfig) {
    ...
    if (this._xConfig.defaultProtocol) {
      const registered = await neutron.appSetAsDefaultProtocolClient(
        this._xConfig.defaultProtocol
      );
      xLog.dbg(
        `Default protocol ${this._xConfig.defaultProtocol}:// registration: ${registered}`
      );
    }

    const isGraphical = await neutron.isGraphical();
    if (isGraphical && this.#config.useRealms) {
    ...
    if (isGraphical) {
      if (this._xConfig.singleInstance) {
        const gotTheLock = await neutron.appRequestSingleInstanceLock();
        if (!gotTheLock) {
          await neutron.appQuit();
        } else {
          await neutron.appOnSecondInstance(
            async (event, rawArgs, workingDir) => {
              try {
                await neutron.wmFocusApp();
      ...
      await neutron.appOnWindowAllClosed(async () => {
        if (this._ignoreCloseRequests) {
          neutron.msgBox('Waiting for application restart...');
          return;
        }
        if (this._busClient && this._busClient.isConnected()) {
      ...
      // Specific for macOS
      await neutron.appOnOpenFile((ev, filePath) => {
        if (this._busClient && this._busClient.isConnected()) {
      ...
      await neutron.appOnOpenUrl((ev, url) => {
        if (this._busClient && this._busClient.isConnected()) {
      ...
      for (const blockerType of this._xConfig.powerSaveBlockers) {
        this._powerSaveBlockerIds.push(
          await neutron.powerSaveBlockerStart(blockerType)
        );
      }

      if (this._xConfig.powerMonitorSweeper) {
        this.#idleStateInterval = setInterval(async () => {
          const idleState = await neutron.powerMonitorGetSystemIdleState(30);
          switch (idleState) {
            case 'locked':
  ...
  async selectRealm(config) {
    ...
    let skipEnv = false;
    const isGraphical = await neutron.isGraphical();
    if (isGraphical && config.useRealms) {
      await neutron.appWhenReady();
  ...
  *_terminate(next) {
    ...
    const isGraphical = yield neutron.isGraphical();
    if (isGraphical) {
      for (const powerSaveId of this._powerSaveBlockerIds) {
        yield neutron.powerSaveBlockerStop(powerSaveId);
      }
    }
  }
  ...
  *_startQuests(xBus, start, next) {
    ...
    if (isGraphical) {
      ...
      if (isGraphical && appArgs['relaunch-reason']) {
        const appName = yield neutron.appGetName();
        let message = `${appName} is restarting, please wait...`;
        if (appArgs) {
          switch (appArgs['relaunch-reason']) {
            case 'client-connection':
              message = `${appName} is restarting after a connection lose, please wait...`;
              break;
            case 'server-restart':
              message = `${appName} is restarting after a server restart, please wait...`;
              break;
          }
        }

        neutron.msgBox(message);
      }
    }

    if (isGraphical) {
      for (const proto of Object.keys(this._xConfig.protocols)) {
        yield neutron.protocolHandle(
          proto,
          async (req) => await this._notifyProtocol(req)
        );
      }
    }
  ...
  *boot(start, isTesting, next) {
    ...
    const isGraphical = yield neutron.isGraphical();
    if (isGraphical && !this._xConfig.disableGoblinWM) {
...
module.exports = async (start = null) => {
  ...
  if (isGraphical) {
    ...
    await neutron.protocolRegisterSchemesAsPrivileged([
    ...
    if (process.platform === 'linux' && process.env.APPIMAGE) {
      ...
      await neutron.appSetDesktopName(desktopName);
    }
  }

  const initialConfig = await initialConfigLoader(appArg, isGraphical);
  if (!initialConfig) {
    return;
  }
  const host = new Host();
