...
class Host extends EventEmitter {
  ...
  async load(initialConfig, beforeReady) {
    ...
    if (isElectron) {
      ...
      // Specific for macOS
      this._app.on('activate', (ev, hasVisibleWindows) => {
        if (this._busClient && this._busClient.isConnected()) {
          this._notifyActivate(hasVisibleWindows);
        }
      });
      ...
      if (this._xConfig.powerMonitorSweeper) {
        let state = 'active';
        /* Here we use an interval because the events are not supported on
         * Linux hosts, see https://www.electronjs.org/docs/latest/api/power-monitor
         */
        this.#idleStateInterval = setInterval(() => {
          ...
          switch (idleState) {
            case 'locked':
              this._notifyPowerMonitorLock('locked').catch((ex) =>
                xLog.err(ex.stack || ex.message || ex)
              );
              break;
            case 'active':
            case 'unknown':
              this._notifyPowerMonitorLock('unlocked').catch((ex) =>
                xLog.err(ex.stack || ex.message || ex)
              );
              break;
          }
  ...
  *_notifyProtocol(req, next) {
    ...
    try {
      ...
      if (result?.data) {
        ...
        const stream = fse.createReadStream(location);
  ...
  async _notifyPowerMonitorLock(status) {
    if (!this._xConfig.powerMonitorSweeper) {
      return;
    }
    if (!this._busClient || !this._busClient.isConnected()) {
    ...
    if (status === 'unlocked') {
      ...
      if (syncClientEnabled) {
        const sync = require('xcraft-core-goblin/lib/sync/index.js')();
        for (const db of dbs) {
          sync.sync(db); /* fire and forget */
        }
      }
      return;
    }

    if (status === 'locked' && !this.#sweeping) {
      ...
      for (const db of dbs) {
        ...
        /* Stop sweeping ASAP when the system is unlocked */
        if (!this.#sweeping || idleState !== 'locked') {
          break;
        }
        await new Promise((resolve, reject) =>
          this._busClient.command.send('cryo.sweep', {dbs: [db]}, null, (err) =>
            err ? reject(err) : resolve()
          )
        );
        /* Tick, otherwise we can't quickly capture the getSystemIdleState */
  ...
  _notifyActivate(hasVisibleWindows) {
    if (!this._xConfig.activateQuest) {
      return;
    }
    this._busClient.command.send(
      this._xConfig.activateQuest,
      {hasVisibleWindows},
      null,
      (err) => {
        if (err) {
...
module.exports = async (start = null) => {
  ...
  const beforeReady = (config) => {
    if (!isElectron) {
      return;
    }

    /* HACK: We need the base config because the protocols cannot be
     * registered properly after the ready event. In all other cases
     * the module xcraft-core-etc __must__ be used.
     * This base config.js file is handled by the init-env.js script.
     */
    const baseConfig = path.join(config.projectPath, 'config.js');
    if (!fse.existsSync(baseConfig)) {
      return;
    }

    let xConfig = require(baseConfig);
    xConfig = xConfig[config.appId] || xConfig.default;
    if (!xConfig) {
      return;
    }

    const {protocol} = electron;
    const {protocols} = xConfig['xcraft-core-host'];
    if (!protocols) {
      return;
    }
    ...
    for (const scheme of Object.keys(protocols)) {
      protocol.registerSchemesAsPrivileged([
        {scheme, privileges: {standard: true, corsEnabled: true}},
      ]);
    }
  };

  const initialConfigLoader = require('./initialConfigLoader.js');
  const initialConfig = await initialConfigLoader(
    appArg,
    isElectron,
    beforeReady
  );
