...
function tryInitializeFromEnvironment(xLog) {
  try {
...
const tryInitializeFromInternetSettings = watt(function* (xLog) {
  try {
...
const tryInitializeFromPacFileWindows = watt(function* (xLog, next) {
  try {
    const registry = require('node-windows-registry');
...
const tryInitializeFromPacFile = watt(function* (xLog) {
  try {
    if (os.platform() === 'win32') {
      return yield tryInitializeFromPacFileWindows(xLog);
    }
  } catch (ex) {
    xLog.warn(
      `cannot initialize network stack for proxy from pac file: ${
...
const initializeNetworkStackForProxy = watt(function* (xLog) {
  try {
    if (
      tryInitializeFromEnvironment(xLog) ||
      (yield tryInitializeFromInternetSettings(xLog)) ||
      (yield tryInitializeFromPacFile(xLog))
    ) {
